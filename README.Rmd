...
`{migraph}` includes functions for measuring networks and their nodes and ties,
...
`{migraph}` includes four special groups of functions, 
each with their own pretty `print()` and `plot()` methods:
measures, memberships, motifs, and models.
Measures are numeric scalars or vectors, memberships categorical,
and motifs and models result in tabular outputs.
...
The package also include functions for returning various censuses of motifs
