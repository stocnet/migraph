...
It offers a broad range of functions to create, import, visualise, analyse, and model
...
The package (originally called `{migraph}`) was intended as a software companion to the book:
...
It also works very well with, often relies on, and in many ways extends existing packages,
such as `{igraph}` and `{sna}`.

## How does manynet help?

`{manynet}` can help with many network analytic tasks,
...
`{manynet}` includes a number of prominent network datasets,
...
If that's not enough, `{manynet}` offers a number of options
...
`{manynet}` offers a large and growing smorgasbord of measures that
...
Please explore [the list of functions](https://snlab-ch.github.io/manynet/reference/index.html) to find out more.
...
`{manynet}` is the only package that offers these testing frameworks 
...
`{manynet}` also includes `autographr()` for one-line plotting graphs with sensible defaults based on their properties.
...
p <- autographr(mpn_elite_usa_advice, "hierarchy") + ggtitle("{manynet} layout")
print(p,vp = vp1) 
```
...
`learnr::run_tutorial("tutorial2", "manynet")`.
