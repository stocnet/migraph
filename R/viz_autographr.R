...
#'   with sensible defaults
#' @param attribute A network attribute (e.g. time variable) used to produce the
#' animated frames.
#' @param animate Would you like the plot to be animated?
#' By default, TRUE, an animated plot is generated with ´gganimate´.
#' If FALSE, plots the frames generated by the attribute side-by-side.
...
#'  activate(edges) %>%
#'  mutate(year = sample(1:4, 10, replace = TRUE)) %>%
#'  autographd(attribute = "year", animate = TRUE)
#' @export
autographd <- function(object, attribute, animate = FALSE, ...) {
  ...
  if (animate == TRUE) {
    ...
    nodes_lst <- lapply(1:length(out), function(i) {
      cbind(igraph::as_data_frame(out[[i]], "vertices"),
            x = layout[[i]][, 1], y = layout[[i]][, 2], frame = i)
    })
    # Create an edge list for each time point
    edges_lst <- lapply(1:length(out), function(i)
      cbind(igraph::as_data_frame(out[[i]], "edges"), frame = i))
    edges_lst <- lapply(1:length(out), function(i) {
