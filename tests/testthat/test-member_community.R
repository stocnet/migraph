test_that("node_kernighanlin algorithm works", {
  expect_s3_class(node_in_partition(mpn_elite_mex), "node_member")
  expect_length(node_in_partition(mpn_elite_mex), 
                network_nodes(mpn_elite_mex))
  expect_false(any(node_in_partition(mpn_elite_mex) > "B"))
})

test_that("node_edge_betweenness algorithm works", {
  expect_s3_class(node_in_betweenness(mpn_elite_mex), "node_member")
  expect_length(node_in_betweenness(mpn_elite_mex), 
                network_nodes(mpn_elite_mex))
})
