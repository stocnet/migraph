...
test_that("one-mode object clustering is reported correctly",{
  expect_equal(round(measure_clustering(om), 2), 0.75)
})

test_that("two-mode object clustering is reported correctly",{
  expect_equal(round(measure_clustering(southern_women), 4), 0.4872)
})
...
expect_equal(round(measure_clustering(mat1, mat2), 4), 0.7143)
})
