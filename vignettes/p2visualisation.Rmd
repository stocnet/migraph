...
As Brandes et al (1999) argue, visualising networks demands thinking about:
...
the `autographr()` function in `{migraph}` is a quick and easy way 
...
![plot of chunk migrapheg](figure/migrapheg-1.png)
...
`{migraph}` works well with both `{ggplot2}` and `{ggraph}` functions 
...
![plot of chunk migraphexample2](figure/migraphexample2-1.png)

`{migraph}` also uses the `{patchwork}` package for arranging
...
![plot of chunk patchwork](figure/patchwork-1.png)

```r
autographr(ison_adolescents) / autographr(ison_algebra)
```

![plot of chunk patchwork](figure/patchwork-2.png)

## Layouts

A range of graph layouts are available across the `{igraph}`, `{graphlayouts}`, and `{migraph}` packages 
...
![plot of chunk ggrapheg](figure/ggrapheg-1.png)
