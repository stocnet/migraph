...
```{r varyclust, exercise = TRUE, exercise.setup = "data", purl = FALSE}
...
plot(node_structural_equivalence(to_named(ison_karateka), cluster = "concor"))
```
...
```{r k-discrete, exercise = TRUE, exercise.setup = "data", purl = FALSE}

```

```{r k-discrete-hint-1, purl = FALSE}
# Let's use the node_structural_equivalence() function and set 'k' to 2

node_structural_equivalence(____, k = ____)
```

```{r k-discrete-hint-2, purl = FALSE}
...
```{r elbowsil, exercise = TRUE, exercise.setup = "data", purl = FALSE}

```

```{r elbowsil-hint-1, purl = FALSE}
...
```{r strict, exercise = TRUE, exercise.setup = "data", purl = FALSE}

```

```{r strict-hint, purl = FALSE}
...
```{r structblock, exercise = TRUE, exercise.setup = "data", warning=FALSE}
str_clu <- node_structural_equivalence(ison_algebra)
...
```{r struct-elites, exercise = TRUE, purl = FALSE}
...
```{r reg-elites, exercise = TRUE, purl = FALSE}
...
```{r auto-elites, exercise = TRUE, purl = FALSE}
