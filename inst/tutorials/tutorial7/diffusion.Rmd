...
autographd(play_diffusion(la, seeds = 16), layout = "grid")
```
...
autographs(play_diffusion(sf, seeds = node_is_min(node_degree(sf)), steps = 10),
           node_color = "Infected", layout = "circle") +
  ggplot2::guides(colour = guide_legend(""))
autographd(play_diffusion(sf, seeds = 16, steps = 10), layout = "circle")
```
...
Well, recall that $R_0 = \frac{\beta}{\gamma}$.
For a network such as ours, the expected number of contacts per node $\beta$
is the average degree of all nodes,
and the average number of days a node remains contagious is 1 divided by the `recovery` parameter.
...
question("Select any true statements you can make about this R-nought result:",
         answer("There will probably be an epidemic.", correct = TRUE),
